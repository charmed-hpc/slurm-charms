...
class TestManager:
    ...
    def test_service_active(self, mock_manager, mock_run, snap_backend, active) -> None:
        """Test the `<manager>.service.active()` method."""
        manager, service = mock_manager
...
class TestSackdManager:
    ...
    def mock_manager(self, fs: FakeFilesystem) -> SackdManager:
        ...
        fs.create_file(
            ...
            contents=textwrap.dedent(
                """
                SACKD_OPTIONS="--conf-server host1:6817,host2:6817"
                """
            ),
        )
        return SackdManager()
    ...
    def test_config_server(self, mock_manager) -> None:
        """Test the `config_server` property."""
        # Get config server addresses.
        assert mock_manager.config_server == ["host1:6817", "host2:6817"]

        # Set new config server addresses.
        mock_manager.config_server = ["controller1,controller2"]
        env = dotenv_values("/etc/default/sackd")

        assert mock_manager.config_server == ["controller1", "controller2"]
        assert "SACKD_OPTIONS" in env
        assert env["SACKD_OPTIONS"] == "--conf-server controller1,controller2"

        # Delete config server address.
        del mock_manager.config_server
        env = dotenv_values("/etc/default/sackd")

        assert mock_manager.config_server is None
        assert "SACKD_OPTIONS" in env
...
class TestSlurmdManager:
    ...
    def mock_manager(self, fs: FakeFilesystem) -> SlurmdManager:
        ...
        fs.create_file(
            ...
            contents=textwrap.dedent(
                """
                SLURMD_OPTIONS="--conf-server host1:6817,host2:6817"
                """
            ),
        )
    ...
    def test_config_server(self, mock_manager) -> None:
        """Test the `config_server` property."""
        # Get configuration server addresses.
        assert mock_manager.config_server == ["host1:6817", "host2:6817"]
        ...
        assert env["SLURMD_OPTIONS"] == "--conf-server controller1,controller2"
